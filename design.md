...
- Provider 与管理端模型身份优先展示已识别供应商的官方 Logo；模型继承所选 Provider 的 Logo。识别失败时回退到 `--chart-series` 九色稳定哈希的字母 monogram。黑色与白色 Logo 分别使用固定浅色或深色承载面，不能依赖当前主题背景碰运气。
- 榜单与战报在缺少 Provider 快照时继续使用 `--chart-series` 九色稳定哈希的模型 monogram；牌桌座位左侧色条按座位顺序使用同一色列，与筹码走势一致。
