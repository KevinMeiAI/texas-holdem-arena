...
这是本项目锁定的视觉系统。所有公开页、赛事解析与管理页在修改前都必须读取本文件；不要为单个页面重新选择主题或字体。
...
- 公开档案、榜单、赛事解析：Index-First。列表和逐手导航直接承担页面结构。
...
- 选中与激活态（tab、手数选择器、导航、下拉选项）使用黄铜指示线或细边框表达，不使用实心黄铜块填充；暗色模式下允许使用深黄铜染色底面增强层级。
- Provider、管理端模型与公开榜单优先展示已识别供应商的官方 Logo；模型继承所选 Provider 的 Logo，榜单从赛事冻结的 competitor revision 解析供应商，不向公共接口暴露 Base URL。识别失败时回退到 `--chart-series` 九色稳定哈希的字母 monogram。黑色与白色 Logo 分别使用固定浅色或深色承载面，不能依赖当前主题背景碰运气。
- 战报等尚未接入 Provider 身份的页面继续使用 `--chart-series` 九色稳定哈希的模型 monogram；牌桌座位左侧色条按座位顺序使用同一色列，与筹码走势一致。
...
- 赛事解析页可以展示横向逐手选择器，但页面根节点不能横向滚动。
- 赛事解析页右侧使用观众视角的“牌局时间线”：保留发牌、盲注、玩家行动、公开结算和赛事关键状态；模型调用、协议审计、底池创建与下注轮内部状态只出现在审计层，不混入观众日志。
- 行动语义跨组件保持一致：黄铜实心圆表示下注/加注，灰色实心圆表示跟注/过牌，灰色空心圆表示弃牌；流程、结果和赛事状态分别使用菱形、绿色菱形和警告三角。
