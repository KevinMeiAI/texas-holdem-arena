...
import { z } from "zod";
import { buildEffectiveSystemPrompt, type CanonicalModelRequest } from "../../../../packages/contracts/src/index.js";
import { createProvider } from "../../../../packages/providers/src/provider-factory.js";
import { preflightProvider } from "../../../../packages/providers/src/provider.js";
...
const modelUpdateSchema = modelSchema.extend({ enabled: z.boolean() }).partial().refine(
  (input) => Object.keys(input).length > 0,
...
function preflightRequest(): CanonicalModelRequest {
  ...
  return {
    ...
    userPayload: {
      ...
      instruction: "Return the legal check action.",
    },
    timeoutMs: 15_000,
  };
}
...
export async function registerAdminModelRoutes(
  ...
  app.post<{ Params: { id: string } }>("/api/admin/providers/:id/test", async (request, reply) => {
    ...
    try {
      ...
      const result = await preflightProvider(createProvider(config), preflightRequest());
  ...
  app.post<{ Params: { id: string } }>("/api/admin/models/:id/preflight", async (request, reply) => {
    ...
    try {
      const result = await preflightProvider(
        createProvider(await context.models.runtimeConfig(request.params.id)),
        preflightRequest(),
      );
