import type { ActionResponse } from "../../../../packages/contracts/src/model-protocol.js";
...
export class ModelProtocolViolation extends Error {
...
export function toDomainAction(hand: HandState, response: ActionResponse): ActionCommand {
  ...
  if (!legal) throw new ModelProtocolViolation("Hand is not accepting a poker action");
  ...
  if (action === "fold" || action === "check" || action === "call" || action === "all_in") {
    if ((action === "fold" && !legal.fold)
      ...
      throw new ModelProtocolViolation(`${action} is not in the legal action set`);
    }
    return { action };
  }
  ...
  if (!bounds || response.amount_to === undefined
    ...
    throw new ModelProtocolViolation(`${action} amount_to is outside the legal range`);
  }
