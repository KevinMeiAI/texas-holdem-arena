...
- 逐轮决策审计：加密保存每次真实请求、历史查询/纠错上下文、原始与解析响应、耗时、用量和错误类型；仅在该手结束后的赛事解析中公开。
- 完整产品界面：观赛室（实时赛事与历史匀速回放）、赛事档案、赛事解析、排行榜，以及 Provider、模型和赛事管理控制室。
- 可传播精彩瞬间：赛事结束后自动生成候选，由管理员选择公开；`/moments` 提供公共索引，单个 Moment 具备双语 crawler metadata 与确定性 1200×675 社交预览图。V1 图片采用 English-first 矢量字形，避免依赖容器字体。
...
`ARENA_PUBLIC_ORIGIN` 是观众实际访问 Arena 的可信 HTTP(S) origin（只含协议、域名和可选端口）。本机保持 `http://127.0.0.1:4100` 即可；部署到公网时必须改成真实 HTTPS origin，Moment 分享链接与社交预览不会读取请求的 `Host` 头来猜测域名。
